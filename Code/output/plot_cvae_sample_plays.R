# Animates play function
# input: dataframe with x,y,jerseNumber, playId, team
...
demov2 = read.csv('/Users/marcrichards/Desktop/BigDataBowl/demov2.csv')

# Animates play function
# input: dataframe with x,y,jerseNumber, playId, team


# function to plot trajectory and field - adapted from Marshall Furnam
...
plot_player_trajectory <- function(demo,play_id,event_time){
  
  
  
  v <- ggplot() +
    geom_point(data = filter(demo,playId == play_id) %>% dplyr::filter(as.character(event) == event_time),
               aes(x=x, y=y, color=team), size = 6) +
    #geom_point(data = demo %>% dplyr::filter(as.character(event) == 'end'),
    #           aes(x=x, y=y, color=team), size = 3) + 
    geom_text(data = filter(demo,playId == play_id) %>% dplyr::filter(as.character(event) == event_time), aes(x=x, y=y, label=jerseyNumber), 
              colour = "white", vjust = 0.36, size = 3) +
    ...
    #facet_wrap(~ playId)
  # scale_color_manual(values = c("#013369", "#825736",  "#D50A0A"))
  return(gg_field_2(v))
  
} 

plot_initial <- plot_player_trajectory(demov2,play_id = 0,event_time = "start")
plot_end0 <- plot_player_trajectory(demov2,play_id = 0,event_time = "end")
plot_end1 <- plot_player_trajectory(demov2,play_id = 1,event_time = "end")
plot_end2 <- plot_player_trajectory(demov2,play_id = 2,event_time = "end")
plot_end3 <- plot_player_trajectory(demov2,play_id = 3,event_time = "end")
plot_end4 <- plot_player_trajectory(demov2,play_id = 4,event_time = "end")
plot_end5 <- plot_player_trajectory(demov2,play_id = 5,event_time = "end")
plot_end6 <- plot_player_trajectory(demov2,play_id = 6,event_time = "end")
plot_end7 <- plot_player_trajectory(demov2,play_id = 7,event_time = "end")
plot_end8 <- plot_player_trajectory(demov2,play_id = 8,event_time = "end")
plot_end9 <- plot_player_trajectory(demov2,play_id = 9,event_time = "end")
